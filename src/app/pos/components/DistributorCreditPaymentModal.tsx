...
import { X, DollarSign, Loader2, CheckCircle, Truck } from 'lucide-react'
import { recordDistributorPayment } from '../actions'
import { formatCurrency } from '@/lib/utils'

interface DistributorCreditPaymentModalProps {
    isOpen: boolean
    onClose: () => void
    distributor: {
        id: string
...
export default function DistributorCreditPaymentModal({ isOpen, onClose, distributor, onSuccess }: DistributorCreditPaymentModalProps) {
    const router = useRouter()
    const [amount, setAmount] = useState('')
    const [description, setDescription] = useState('Abono en caja (Distribuidor)')
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true)
        try {
            await recordDistributorPayment(distributor.id, numAmount, description)
            setStep('success')
            router.refresh()
            setTimeout(() => {
                onSuccess()
            ...
            console.error(error)
            alert('Error al registrar el abono del distribuidor')
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleClose = () => {
        setAmount('')
        setDescription('Abono en caja (Distribuidor)')
        setStep('form')
        onClose()
    }
                        ...
                        {step === 'form' ? (
                            <>
                                <div className="flex justify-between items-center mb-8">
                                    <div>
                                        <p className="text-[10px] font-black text-primary uppercase tracking-[0.4em] mb-2">Crédito Distribuidor</p>
                                ...
                                <div className="bg-white/5 p-6 rounded-3xl border border-white/5 mb-8 flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-black text-white/20 uppercase tracking-widest leading-none mb-1">Distribuidor</p>
                                        <p className="text-lg font-black text-white italic uppercase tracking-tighter">{distributor.name}</p>
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="text-[10px] font-bold text-red-400">Deuda Actual: {formatCurrency(distributor.currentDebt)}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <form onSubmit={handleSubmit} className="space-y-6">
                                    <div>
                                        <label className="block text-[10px] font-black text-white/20 uppercase tracking-[0.3em] mb-4 ml-4">Monto Liquidación</label>
                            ...
                            <div className="text-center py-10">
                                ...
                                <h3 className="text-3xl font-black text-white italic uppercase tracking-tighter mb-2">¡Abono Registrado!</h3>
                                <p className="text-emerald-400 font-bold uppercase text-[10px] tracking-widest">El crédito del distribuidor ha sido actualizado</p>
                            </div>
                        )}
                    </motion.div>
                </div>
